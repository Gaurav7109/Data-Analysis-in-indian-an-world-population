...
select ggl.district,ip.state,ip.population,ggl.growth,ggl.sex_ratio,ggl.literacy from dataset_growth_gender_ratio_literacy 
AS ggl inner join dataset_indian_population as ip on ggl.district = ip.District where ip.state in ('Uttarakhand', 'Rajasthan');
...
select sum(population) as Total_Population_as_on_2011 from dataset_indian_population; 

select sum(2022_population) as Total_Population_as_on_2022 from world_population; 
...
select State, avg(Growth)  as Avg_Growth from dataset_growth_gender_ratio_literacy group by state order by Avg_Growth desc;


-- 15 avg sex ratio of each state:
select State, round(avg(Sex_Ratio), 0) as Avg_Sex_Ratio from dataset_growth_gender_ratio_literacy group by state order by Avg_Sex_Ratio desc;


-- 16 avg literacy rate of each state and return all states having avg literacy rate greater than 85
select State, round(avg(Literacy), 0) as Avg_Literacy_rate from dataset_growth_gender_ratio_literacy 
...
select state, avg(growth) * 100 as Avg_growth from dataset_growth_gender_ratio_literacy group by state order by Avg_growth desc limit 3;

select country, growth_rate from world_population order by growth_rate desc limit 3;


-- 18 5 states showing highest and Lowest sex ratio:

select state, round(avg(Sex_ratio), 2) as Avg_Sex_Ratio from dataset_growth_gender_ratio_literacy group by state order by Avg_Sex_Ratio desc limit 5;

select state, round(avg(Sex_ratio), 2) as Avg_Sex_Ratio from dataset_growth_gender_ratio_literacy group by state order by Avg_Sex_Ratio asc limit 5;


-- 19 top and bottom 3 states in literacy state:

select state, round(avg(Literacy), 0) as Avg_Literacy from dataset_growth_gender_ratio_literacy group by state order by Avg_Literacy desc limit 3;

select state, round(Avg(Literacy), 0) as Avg_Literacy from dataset_growth_gender_ratio_literacy group by state order by Avg_Literacy asc limit 3;
...
union
select * from (
select state, round(avg(Literacy), 0) as Avg_Literacy from dataset_growth_gender_ratio_literacy group by state order by Avg_Literacy limit 3)
 b
) c;
...
select district,state from dataset_growth_gender_ratio_literacy where Lower(State) like "a%" or Lcase(State) like "b%";

select country,capital_city from world_population where Lower(country) like "a%" or Lcase(country) like "b%";
...
(select d1.district, d1.state, d1.sex_ratio/1000 as sex_ratio, d2.Population from dataset_growth_gender_ratio_literacy as d1
join dataset_indian_population as d2 on d1.District = d2.District order by Population desc) c
) d
group by d.state;
...
with literacy_rate_of_each_state as (select state, 100*sum(literate_people)/(sum(illiterate_people)+sum(literate_people)) as literacy_rate from
(
...
(select d1.district, d1.state, d1.literacy/100 as literacy_ratio, d2.Population from dataset_growth_gender_ratio_literacy as d1
join dataset_indian_population as d2 on d1.District = d2.District order by Population desc) c) x
...
select (sum(literate_people)/((sum(literate_people))+(sum(illiterate_people))))*100 as literacy_rate_of_country from 
...
(select d1.district, d1.state, d1.growth as growth, d2.Population from dataset_growth_gender_ratio_literacy as d1
join dataset_indian_population as d2 on d1.District = d2.District order by Population desc) c) x
...
(select District, State, Literacy, rank() over(partition by State order by Literacy desc) rnk from dataset_growth_gender_ratio_literacy)x
